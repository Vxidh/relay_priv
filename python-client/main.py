...
# Assuming NodeClient has been refactored as discussed to manage its own thread/loop
from NodeClient import NodeClient
from RemoteControlAgent import RemoteControlAgent # RemoteControlAgent.py also needs updates as discussed

LOG_FORMAT = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
logging.basicConfig(level=logging.INFO, format=LOG_FORMAT)
logger = logging.getLogger('RPAClient')

DJANGO_RELAY_SERVER_BASE_URL = "http://localhost:8000/api/"
DJANGO_RELAY_SERVER_WS_URL = "ws://localhost:8000/ws/rpa-node/"
REMOTE_CONTROL_AGENT_WS_URL = "ws://localhost:8000/ws/rc-agent/"
...
logger.info(
    f"RPA Client will save received files to: {os.path.abspath(RPA_CLIENT_DOWNLOAD_DIR)}")

NODE_ID = None
ACCESS_TOKEN = None
REFRESH_TOKEN = None
TOKEN_EXPIRY = None
NODE_CLIENT_INSTANCE = None
REMOTE_CONTROL_AGENT_INSTANCE = None
...
def get_or_create_node_id():
    ...
    try:
        ...
        while True:
            node_id_input = simpledialog.askstring(
                "Node ID Required", "Enter your 6-character alphanumeric node ID:", parent=root)
            ...
            if len(node_id_input) == 6 and node_id_input.isalnum():
                ...
                with open(NODE_ID_FILE, "w") as f:
                    f.write(NODE_ID)
                logger.info(
                    f"Using centrally-issued Node ID (from Tkinter input): {NODE_ID}")
                root.destroy()
                return True
            else:
                messagebox.showerror(
                    "Invalid Node ID", "Please enter a valid 6-character alphanumeric node ID.")
    except ImportError:
        logger.warning(
            "Tkinter not available, falling back to CLI input for Node ID.")
    except Exception as e:
        logger.error(
            f"Error in Tkinter Node ID input: {e}, falling back to CLI input.")

    # CLI fallback
    while True:
        user_input = input(
            "Enter your 6-character alphanumeric node ID: ").strip()
        if len(user_input) == 6 and user_input.isalnum():
            NODE_ID = user_input
            with open(NODE_ID_FILE, "w") as f:
                f.write(NODE_ID)
            logger.info(
                f"Using centrally-issued Node ID (from CLI input): {NODE_ID}")
            return True
        else:
...
def load_tokens():
    ...
    if (os.path.exists(ACCESS_TOKEN_FILE) and
        ...
        try:
            ...
            logger.error(
                f"Error loading tokens: {e}. Tokens might be corrupted. Deleting them.")
...
# --- PKCE OAuth2 Flow ---
# Set this to your DOT Application's client_id
OAUTH2_CLIENT_ID = "bc0xM6l96NYdvXghcapdtkP7hiSKHSeCFAH5E9T3"
...
# Ensure these scopes match your Django OAuth2 Provider setup
OAUTH2_SCOPES = "rpa:connect rpa:commands"
    ...
    if load_tokens():
        if not is_token_expired():
            ...
            if refresh_access_token():
                ...
                logger.warning(
                    "Failed to refresh token. Will try PKCE authorization.")
    logger.info(
        "No valid tokens found or refresh failed. Initiating PKCE OAuth2 flow.")
...
def refresh_access_token():
    ...
    try:
        ...
        save_tokens(token_data["access_token"],
                    token_data["refresh_token"], token_data["expires_in"])
        logger.info("Access token refreshed successfully.")
        return True
    except requests.exceptions.RequestException as e:
        logger.error(
            f"Failed to refresh access token: {e}. Response: {response.text if 'response' in locals() else 'N/A'}")
...
def pkce_authorization_flow():
    ...
    # 60 seconds timeout
    while server.auth_code is None and (time.time() - timeout_start < 60):
        time.sleep(0.5)
    ...
    try:
        ...
        logger.error(
            f"Failed to exchange code for tokens: {e}. Response: {response.text if 'response' in locals() else 'N/A'}")
        clear_tokens()
        return False
    except Exception as e:
        logger.error(
...
def main():
    global NODE_CLIENT_INSTANCE, REMOTE_CONTROL_AGENT_INSTANCE, NODE_ID, ACCESS_TOKEN
    clear_tokens()
    ...
    # Initialize NodeClient (RPA C&C)
    NODE_CLIENT_INSTANCE = NodeClient(
        server_url=f"{DJANGO_RELAY_SERVER_WS_URL}{NODE_ID}/",
        node_id=NODE_ID,
        access_token=ACCESS_TOKEN,
        download_dir=RPA_CLIENT_DOWNLOAD_DIR,
        initial_metadata=initial_node_metadata
    )

    # Initialize RemoteControlAgent
    REMOTE_CONTROL_AGENT_INSTANCE = RemoteControlAgent(
        rc_server_ws_url=f"{REMOTE_CONTROL_AGENT_WS_URL}{NODE_ID}/",
        node_id=NODE_ID,
        access_token=ACCESS_TOKEN,
        command_dispatcher_instance=NODE_CLIENT_INSTANCE.dispatcher
    )

    # Start WebSocket connections in separate threads
    # NodeClient connection (CORE functionality)
    # The NodeClient's connect() method now spawns its own thread and asyncio loop internally.
    NODE_CLIENT_INSTANCE.connect()
    logger.info("Main: NodeClient connection requested.")

    # RemoteControlAgent connection (OPTIONAL feature)
    # The RemoteControlAgent's start() method also spawns its own thread and asyncio loop internally.
    REMOTE_CONTROL_AGENT_INSTANCE.start()
    logger.info("Main: RemoteControlAgent connection requested.")

    # --- Connection Check Logic ---
    logger.info("Main: Waiting for NodeClient (RPA C&C) WebSocket to connect...")
    timeout_start = time.time()
    connection_timeout_seconds = 20 # Give enough time for both loop creation and connection

    connected = False
    while not connected and (time.time() - timeout_start < connection_timeout_seconds):
        if NODE_CLIENT_INSTANCE._websocket_loop is None or NODE_CLIENT_INSTANCE._websocket_loop.is_closed():
            logger.debug("Main: NodeClient loop not yet ready or closed, waiting...")
            time.sleep(0.5)
            continue # Continue loop to re-check

        try:
            connected = asyncio.run_coroutine_threadsafe(
                NODE_CLIENT_INSTANCE.is_connected(),
                NODE_CLIENT_INSTANCE._websocket_loop
            ).result(timeout=1) # Wait up to 1 second for the result of is_connected()
            logger.debug(f"Main: NodeClient connection status: {connected}")
        except asyncio.TimeoutError:
            logger.debug("Main: Timeout checking NodeClient connection status (is_connected() call).")
            connected = False
        except Exception as e:
            logger.error(f"Main: Error checking NodeClient connection status: {e}", exc_info=True)
            connected = False
        
        if not connected:
            time.sleep(0.5) # Sleep before next check if not connected

    if connected:
        logger.info(
            "NodeClient (RPA C&C) WebSocket connected. RPA Client is running.")
        try:
            ...
            # Ensure both client instances are stopped gracefully on exit
            if NODE_CLIENT_INSTANCE:
                NODE_CLIENT_INSTANCE.stop() # This will stop its own thread/loop
            if REMOTE_CONTROL_AGENT_INSTANCE:
                REMOTE_CONTROL_AGENT_INSTANCE.stop() # This will stop its own thread/loop
            logger.info("All client components stopped.")
    else:
        logger.error(
            "Failed to connect NodeClient (RPA C&C) WebSocket after waiting. RPA Client will exit.")
        # If NodeClient fails, RC agent is also not useful. Stop it if it started.
        if REMOTE_CONTROL_AGENT_INSTANCE:
            REMOTE_CONTROL_AGENT_INSTANCE.stop()
